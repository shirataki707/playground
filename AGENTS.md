...
| テーマ・スペーシングトークン | `playground.android.library.compose` |
...
- `playground.android.feature` は Compose 依存・Navigation3・material-icons-core・`:core:navigation`・`:core:theme` を自動追加し、パスに `contract` セグメントを含む全 feature モジュールを依存に自動注入する
...
:core:theme                      # PlaygroundTheme / PlaygroundSpacing
